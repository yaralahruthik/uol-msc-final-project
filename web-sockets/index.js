...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (message) => {
    ...
    switch (command) {
      case 'READ_ONE':
        ws.send(JSON.stringify(todoStore.find((todo) => todo.id === +args.id)));
        break;

      case 'READ_100':
        ws.send(JSON.stringify(todoStore.slice(-100)));
        break;
      ...
      case 'INSERT_ONE':
        ...
        todoStore.push(newTask);
        ws.send('Data entry inserted successfully!');
        break;
